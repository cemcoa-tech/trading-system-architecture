# strategies/treasury_30y_eom.py
"""
strategies/treasury_30y_eom.py
──────────────────────────
...
Entry:
    - 6 trading days before end of month (e.g., 6 days)
...
class Treasury30YEOMStrategy(BaseStrategy):
    """
    Treasury 30-Year Bond End-of-Month calendar strategy.
    
    Simple calendar-based entry/exit with no technical indicators.
    Holds position from 6 days before month-end until first day of next month.
    """
    ...
    def generate_signal(self, df: pd.DataFrame, current_pos: int) -> Signal:
        ...
        if in_position and current_pos == 0:
            ...
            if current_pos > 0:
                ...
                self._position_state["entry_price"] = close_price
        ...
        if current_pos > 0 and is_first_td:
            self.log.info("EXIT: First trading day of month")
            
            return Signal(
                signal_type="EXIT_LONG",
                reason="Exit: First trading day of month",
                close_price=close_price,
                indicators=indicators,
                meta={**meta, "exit_type": "first_td"},
            )
    ...
    def get_position_size(self, signal: Signal) -> int:
        """Fixed position size (typically 1 contract for ZB)."""
