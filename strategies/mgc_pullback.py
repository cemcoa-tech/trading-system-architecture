...
class MGCPullbackStrategy(BaseStrategy):
    ...
    Enters on RSI(2) dips within an uptrend; exits on SMA(37) cross.
    """
    ...
    def __init__(
        ...
        self._exit_ma_len: int = p.get("exit_ma_length", 37)
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """Add SMA200, SMA37, RSI(2)."""
        df["SMA200"] = sma(df["close"], self._trend_len)
        df["SMA37"] = sma(df["close"], self._exit_ma_len)
        df["RSI2"] = wilder_rsi(df["close"], self._rsi_len)
        self.log.info(
            "Last bar  date=%s  close=%.2f  SMA200=%.2f  SMA37=%.2f  RSI2=%.2f",
            df.iloc[-1]["date"],
            df.iloc[-1]["close"],
            df.iloc[-1]["SMA200"],
            df.iloc[-1]["SMA37"],
    ...
    def generate_signal(self, df: pd.DataFrame, current_pos: int) -> Signal:
        ...
        Decision logic:
            ...
            Long + close > SMA37      →  EXIT_LONG
            Otherwise                  →  NONE
        """
        last = df.iloc[-1]
        close = float(last["close"])
        sma200 = float(last["SMA200"])
        sma37 = float(last["SMA37"])
